...
[Таблица](./table-dfa.md) для ДКА

## НКА

![NFA](./nfa.svg)

[Таблица](./table-nfa.md) для НКА

## ПКА

![AFA](./afa.svg)

[Таблица](./table-afa.md) для ПКА
...
Избавимся от неоднозначности

Поскольку некоторые слова могут распознаваться и X и Y, добавим отрицательный предпросмотр. 
Он запретит переходить к Y, если еще можно читать X
...
```
X = ((a[ab]|cc)*aba(aaa|bcc)*)
Y = ((abac|(cc)*)(b|ca))
```

Каких-то дальнейших упрощений регулярки я не вижу

То есть итоговое расширенное регулярное выражение:

```
^ ((a[ab]|cc)*aba(aaa|bcc)*)* (?! ((a[ab]|cc)*aba(aaa|bcc)*) ((abac|(cc)*)(b|ca))* $ ) ((abac|(cc)*)(b|ca))* $
```
